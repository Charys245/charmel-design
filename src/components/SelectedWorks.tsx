    ...
    <div className="group/row relative">
      <div className="mx-auto max-w-400 px-6 md:px-10 mb-8 flex items-end justify-between gap-6">
        <div>
          ...
          <p className="text-cream/60 text-sm md:text-base mt-2 max-w-md">{row.description}</p>
        </div>
        <div className="hidden md:flex items-center gap-2">
          <button
            onClick={() => scrollBy(-1)}
            aria-label="Previous"
            className="h-11 w-11 rounded-full border border-cream/15 hover:border-yellow hover:text-yellow text-cream/70 transition-all flex items-center justify-center"
          >
            ←
          </button>
          <button
        ...
        {items.map((p, i) => (
          ...
          >
            <div className="relative aspect-4/5 overflow-hidden bg-cream/3">
              ...
              />
              <div className="absolute top-4 left-4 right-4 flex justify-between text-[10px] uppercase tracking-[0.25em] text-cream/80 mix-blend-difference">
            ...
            <figcaption className="mt-4 flex items-start justify-between gap-4">
              <div>
                <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40 mb-1">{p.category}</p>
                <p className="font-display text-base md:text-lg text-cream">{p.client}</p>
              </div>
              <span className="text-cream/40 group-hover/card:text-yellow group-hover/card:translate-x-1 transition-all font-display text-lg">
                ↗
              </span>
            </figcaption>
          </figure>
...
export function SelectedWorks() {
  ...
  return (
    <section className="relative py-32 md:py-40 border-t border-cream/10 bg-ink text-cream overflow-hidden">
      <div
        aria-hidden
      ...
      >
        <div className="col-span-12 md:col-span-7">
          <p className="text-[11px] uppercase tracking-[0.3em] text-cream/50 mb-6">— Portfolio</p>
          <h2 className="font-display font-medium leading-[0.95] tracking-[-0.03em] text-[clamp(2.25rem,6vw,5.5rem)]">
            Selected <em className="not-italic text-yellow">Visual</em> Works.
          </h2>
        </div>
        <div className="col-span-12 md:col-span-5 md:pt-6">
          <p className="text-cream/65 text-base md:text-lg max-w-md">
